    ...
    <div class="header__wrapper page__header-wrapper container">
        <div class="header__logo-wrapper page__logo-wrapper">
            <a class="header__logo-link header__logo-link--active">
                <img class="header__logo" src="/assets/img/logo.svg" alt="Логотип readme" width="172" height="32">
            </a>
        ...
        <section class="authorization">
            ...
            <form class="authorization__form form" action="/" method="post">
                <div class="authorization__input-wrapper form__input-wrapper">
                    <?= !empty($errors['email']) ? '<div class="form__input-section form__input-section--error">' : '' ?>
                    <input class="authorization__input authorization__input--login form__input" type="email" name="email" placeholder="Электронная почта" value="<?= !empty($_POST['email']) ? $_POST['email']: '' ?> <?= empty($_POST['email']) && !empty($_COOKIE['visit']) ? $_COOKIE['visit'] : ''?>">
                    <svg class="form__input-icon" width="19" height="18">
                        <use xlink:href="#icon-input-user"></use>
                    </svg>
                    <label class="visually-hidden">Электронная почта</label>
                    <?= !empty($errors['email']) ? '</div>' : '' ?>
                    <span class="form__error-label form__error-label--login"><?=$errors['email']?></span>
                </div>

                <div class="authorization__input-wrapper form__input-wrapper">
                    <?= !empty($errors['password']) ? '<div class="form__input-section form__input-section--error">' : '' ?>
                    <input class="authorization__input authorization__input--password form__input" type="password" name="password" placeholder="Пароль" value="<?= !empty($_POST['password']) ? $_POST['password']: '' ?>">
